<script lang="ts">
	...
	let contact: ContactContent =
		data.authenticated && data.content
			? structuredClone(data.content.contact)
			: ({} as ContactContent);
	...
	const addAddressLine = () => {
		contact = {
			...contact,
			address: { ...contact.address, lines: [...contact.address.lines, ''] }
		};
	};
	const removeAddressLine = (index: number) => {
		...
		contact = {
			...contact,
			address: { ...contact.address, lines: remaining.length ? remaining : [''] }
		};
	};
...
{#if data.authenticated}
	<AdminShell
		...
		subtitle="Tekst, adres, telefoon, e-mail en bedrijfsgegevens."
		active="contact"
	>
		<form id={formId} method="post" action="?/save" use:enhance={saveEnhancer} class="space-y-6">
			...
			<section class="rounded-3xl border border-black/10 bg-white p-6">
				...
				<div class="mt-5 space-y-4">
					<div>
						<label for="contact-title" class="block text-sm font-medium text-neutral-700"
							>Titel</label
						>
						<input
							id="contact-title"
					...
					<div>
						<label for="contact-description" class="block text-sm font-medium text-neutral-700"
							>Beschrijving</label
						>
						<textarea
					...
					{#each contact.address.lines as line, index}
						<div class="flex items-center gap-3">
							<input
								...
								bind:value={contact.address.lines[index]}
							/>
							<button
								type="button"
								class="text-xs tracking-[0.2em] text-red-500 uppercase"
								on:click={() => removeAddressLine(index)}>Verwijder</button
							>
						</div>
					{/each}
					<button
						...
						on:click={addAddressLine}>+ Adresregel</button
					>
					<div class="grid gap-4 md:grid-cols-2">
						<div>
							<label for="contact-phone" class="block text-sm font-medium text-neutral-700"
								>Telefoon</label
							>
							<input
								id="contact-phone"
						...
						<div>
							<label for="contact-email" class="block text-sm font-medium text-neutral-700"
								>E-mail</label
							>
							<input
								id="contact-email"
				...
				<div class="mt-5 space-y-4">
					{#each contact.businessDetails as detail, index}
						<div class="flex items-center gap-3">
							<input
								class="w-full rounded-2xl border border-black/10 px-4 py-3"
								bind:value={contact.businessDetails[index]}
							/>
							<button
								type="button"
								class="text-xs tracking-[0.2em] text-red-500 uppercase"
								on:click={() => removeBusinessDetail(index)}>Verwijder</button
							>
						</div>
					{/each}
					<button
						type="button"
						class="rounded-full border border-black/10 px-4 py-2 text-xs tracking-[0.24em] uppercase"
						on:click={addBusinessDetail}>+ Regel</button
					>
				</div>
			</section>
		</form>
