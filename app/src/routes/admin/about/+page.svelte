<script lang="ts">
	...
	let about: AboutContent =
		data.authenticated && data.content ? structuredClone(data.content.about) : ({} as AboutContent);
	...
	const addFeature = () => {
		about = {
			...about,
			features: [...about.features, { icon: 'Globe', title: '', description: '' }]
		};
	};
...
{#if data.authenticated}
	<AdminShell
		...
		subtitle="Introductie, bureauverhaal en Maria’s profiel."
		active="about"
	>
		<form id={formId} method="post" action="?/save" use:enhance={saveEnhancer} class="space-y-6">
			...
			<section class="rounded-3xl border border-black/10 bg-white p-6">
				...
				<div class="mt-5 space-y-4">
					<div>
						<label for="about-intro-label" class="block text-sm font-medium text-neutral-700"
							>Intro label</label
						>
						<input
							id="about-intro-label"
							class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3"
							bind:value={about.introLabel}
						/>
					</div>
					<div>
						...
						<input
							id="about-title"
							class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3"
							bind:value={about.title}
						/>
					</div>
					<div>
						<label for="about-description" class="block text-sm font-medium text-neutral-700"
							>Beschrijving</label
						>
						<textarea
					...
					{#each about.features as feature, index}
						<div class="rounded-2xl border border-black/10 p-4">
							...
							<button
								...
								on:click={() => removeFeature(index)}>Verwijder blok</button
							>
						</div>
					{/each}
					<button
						...
						on:click={addFeature}>+ Blok</button
					>
				</div>
			</section>
			...
			<section class="rounded-3xl border border-black/10 bg-white p-6">
				...
				<div class="mt-5 space-y-4">
					<div>
						<label for="team-title" class="block text-sm font-medium text-neutral-700"
							>Sectietitel</label
						>
						<input
							id="team-title"
							class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3"
							bind:value={about.team.title}
						/>
					</div>
					<div>
						<label for="team-description" class="block text-sm font-medium text-neutral-700"
							>Beschrijving</label
						>
						<textarea
							id="team-description"
							rows="3"
							class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3"
							bind:value={about.team.description}
						></textarea>
					</div>
					{#each about.team.members as member, index}
						<div class="rounded-2xl border border-black/10 p-4">
							...
							<button
								type="button"
								class="mt-3 text-xs tracking-[0.2em] text-red-500 uppercase"
								on:click={() => removeMember(index)}>Verwijder profiel</button
							>
						</div>
					{/each}
					<button
						type="button"
						class="rounded-full border border-black/10 px-4 py-2 text-xs tracking-[0.24em] uppercase"
						on:click={addMember}>+ Profiel</button
					>
				</div>
			</section>
		</form>
