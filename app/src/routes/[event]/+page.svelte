...
<article class="bg-[#f6f8fb] px-4 pt-8 text-black sm:px-6">
	...
	>
		...
		<div
			id="project-story"
			class="prose prose-lg mt-7 max-w-none leading-[1.8] text-neutral-700 md:col-span-6 md:col-start-5 md:mt-0 prose-p:mt-0 prose-p:mb-[1.3em]"
		>
	...
	{/if}
	<section
		class="mx-auto flex max-w-[1280px] flex-col items-start justify-between gap-8 border-t border-black/15 py-[72px] md:flex-row md:items-end"
	>
		<div>
			<p class="m-0 text-xs font-medium tracking-[0.08em] text-secondary uppercase">
				Uw eigen project
			...
			<h2
				class="mt-3 max-w-[700px] text-[clamp(2rem,4vw,4.2rem)] leading-[.98] font-medium tracking-[-.055em]"
			>
				Een eerste idee begint met een gesprek.
			</h2>
		</div>
		<a
			class="border border-secondary px-4 py-3 text-sm text-black no-underline transition-colors hover:bg-secondary hover:text-white"
			href="/contact">Neem contact op</a
		>
	</section>
</article>
...
{#if lightboxOpen}
	...
	>
		...
		<div class="relative z-10 max-h-full max-w-[min(1200px,100%)]">
			<button
				class="absolute top-2.5 right-2.5 grid h-10 w-10 place-items-center border-0 bg-white text-black"
				type="button"
				on:click={closeLightbox}
			...
			/>{#if gallery.length > 1}<button
					class="absolute top-1/2 left-0 grid h-10 w-10 -translate-y-1/2 place-items-center border-0 bg-white text-black sm:left-2.5"
					type="button"
					on:click={previousImage}
					aria-label="Vorige afbeelding"><ChevronLeft size={24} /></button
				><button
					class="absolute top-1/2 right-0 grid h-10 w-10 -translate-y-1/2 place-items-center border-0 bg-white text-black sm:right-2.5"
					type="button"
					on:click={nextImage}
