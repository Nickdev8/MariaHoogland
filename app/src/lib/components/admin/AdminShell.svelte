<script lang="ts">
	...
	export let subtitle = '';
	export let active: 'dashboard' | 'home' | 'about' | 'portfolio' | 'contact' | 'footer' =
		'dashboard';
		...
		<div class="mx-auto max-w-7xl px-6 pb-4">
			<nav class="flex flex-wrap gap-2">
				{#each links as link}
					<a
						href={link.href}
						class={`rounded-full px-4 py-2 text-xs font-semibold tracking-[0.22em] uppercase transition ${
							active === link.key
