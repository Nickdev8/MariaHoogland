...
>
	...
	>
		{#each images as image}
			<div class="flex-shrink-0" style="width: {slideWidthPct}%; padding: 0 4px;">
				<img src={image.src} alt={image.alt} loading="lazy" class="h-48 w-full object-cover" />
			</div>
		{/each}
	</div>
	...
	<button
		on:click={prev}
		class="absolute top-1/2 left-2 z-10 flex h-8 w-8 -translate-y-1/2 items-center justify-center bg-white/70 text-secondary shadow-sm transition hover:bg-white hover:text-textcolor"
		aria-label="Vorige"
	>
		<ChevronLeft size={18} />
	</button>
	...
	<button
		on:click={next}
		class="absolute top-1/2 right-2 z-10 flex h-8 w-8 -translate-y-1/2 items-center justify-center bg-white/70 text-secondary shadow-sm transition hover:bg-white hover:text-textcolor"
		aria-label="Volgende"
	>
...
{#if totalSlides > 1}
	<div class="mt-4 flex justify-center gap-2">
		{#each { length: totalSlides } as _, i}
			<button
				on:click={() => goTo(i)}
				class="h-1.5 transition-all duration-300 {currentIndex === i
					? 'w-6 bg-secondary'
